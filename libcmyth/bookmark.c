...
int
cmyth_set_bookmark(cmyth_conn_t conn, cmyth_proginfo_t prog, long long bookmark)
{
	char *buf;
	unsigned int len = CMYTH_TIMESTAMP_LEN + CMYTH_LONGLONG_LEN + 18;
	char resultstr[3];
	...
	sprintf(buf, "%s %ld %s %lld %lld", "SET_BOOKMARK",
		prog->proginfo_chanId, start_ts_dt, bookmark >> 32,
		(bookmark & 0xffffffff));
	pthread_mutex_lock(&mutex);
	if ((err = cmyth_send_message(conn, buf)) < 0) {
		cmyth_dbg(CMYTH_DBG_ERROR,
	...
	count = cmyth_rcv_length(conn);
	if ((r = cmyth_rcv_string(conn, &err, resultstr, sizeof(resultstr),
				  count)) < 0) {
		cmyth_dbg(CMYTH_DBG_ERROR,
